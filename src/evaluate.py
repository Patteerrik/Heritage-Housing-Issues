...
def regression_performance(model):
    """Evaluate model performance with train-test data."""
    # Load train-test data
    X_train = pd.read_csv(
        "jupyter_notebooks/outputs/pipelines/"
        "train-test/v1/X_train.csv"
    )
    X_test = pd.read_csv(
        "jupyter_notebooks/outputs/pipelines/"
        "train-test/v1/X_test.csv"
    )
    y_train = pd.read_csv(
        "jupyter_notebooks/outputs/pipelines/"
        "train-test/v1/y_train.csv"
    ).squeeze()
    y_test = pd.read_csv(
        "jupyter_notebooks/outputs/pipelines/"
        "train-test/v1/y_test.csv"
    ).squeeze()
...
def regression_evaluation(X, y, model):
    """Compute and display metrics for dataset."""
...
def regression_evaluation_plots(model, alpha_scatter=0.5):
    """
    Display scatterplots for actual vs predicted values.
    """
    # Load train-test data
    X_train = pd.read_csv(
        "jupyter_notebooks/outputs/pipelines/"
